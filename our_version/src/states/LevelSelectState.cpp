...
void LevelSelectState::onEnter() {
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        std::string textureId = "btn_level" + std::to_string(i + 1);
        btn.sprite.setTexture(rm.getTexture(textureId));
        
        sf::FloatRect bounds = btn.sprite.getLocalBounds();
        btn.sprite.setOrigin(bounds.width / 2.0f, bounds.height / 2.0f);
        btn.sprite.setScale(300.0f / bounds.width, 100.0f / bounds.height);
        
        btn.sprite.setPosition(winSize.x / 2.0f, winSize.y * 0.4f + i * 150.0f);
        ...
        btn.text.setPosition(btn.sprite.getPosition());
...
void LevelSelectState::update(float dt) {
    ...
    for (auto& btn : buttons) {
        if (btn.sprite.getGlobalBounds().contains(mousePos)) {
            btn.sprite.setColor(sf::Color(200, 200, 200, 255));
        } else {
